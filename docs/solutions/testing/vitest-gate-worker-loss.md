...
manifest. The gate implementation is in `scripts/vitest-gate.mjs:11-111`, and
...
worker loss, caller overrides, and a short run in
`tests/vitest-gate.test.js:24-80`.
...
`close` in `scripts/run-vitest-gate.mjs:43-45`, after stdout and stderr close.
...
filters or overrides in `scripts/vitest-gate.mjs:5-17`; focused development
runs use `npm run test:focused -- <file or filter>`. It also rejects worker-loss
text, failed tests, missing summaries, unaccounted tests, and unexpected
file/test totals in `scripts/vitest-gate.mjs:34-111`. Full runs on Node 22.23.1 passed with 139
passed test files, 8 skipped files, 1,303 passed tests, 18 skipped tests, and
1,321 total tests.
