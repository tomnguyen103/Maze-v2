...
A daily internal job signs the checkpoint timestamp and
`(max(id), row_hash)` with HMAC-SHA256, then writes the authenticated JSON under
a derived unique key in an S3-compatible bucket with Object Lock compliance
retention. Writes require a create-only precondition.
...
The chain verifier enumerates retained object versions, rejects delete markers,
and validates the database chain, checkpoint schema, HMAC, and anchored
row/hash pair. It distinguishes a broken chain, a bad checkpoint, and an
operational inability to verify.
...
- Migration 0013 revokes PUBLIC append execution. The immediately following
  `npm run audit:provision` command is an explicit deployment gate that grants
  the named login the constrained runtime role and proves the boundary.
- Object Lock configuration is a deployment gate; local tests use the same
  append and sink interfaces without creating paid resources.
